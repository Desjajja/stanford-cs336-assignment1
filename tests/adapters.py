...
def run_transformer_block(
    ...
    # weights["ffn.w1.weight"] = weights["ffn.w1.weight"].T
    # weights["ffn.w2.weight"] = weights["ffn.w2.weight"].T
    # weights["ffn.w3.weight"] = weights["ffn.w3.weight"].T
...
def run_rmsnorm(
    ...
    rms_norm.load_state_dict(state_dict={"weight": weights})
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    trainer = BPETrainer(num_processes=8, special_tokens=special_tokens)
    max_merges = vocab_size - 256 - len(special_tokens)
    vocab, merges = trainer.train(input_path, max_merges)
    return vocab, merges
